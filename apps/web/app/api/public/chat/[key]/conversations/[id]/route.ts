...
/** The visitor's own thread (to receive human replies). */
export async function GET(req: NextRequest, { params }: { params: Promise<{ key: string; id: string }> }) {
  ...
  try {
    ...
    await enforceRateLimits(channel, `${visitorId}:poll`, clientIp(req));
    const thread = await publicThread(db(), channel.organization_id, id, visitorId, channel.id);
    return NextResponse.json(thread, { headers: { "x-request-id": requestId, "cache-control": "no-store" } });
  } catch (e) {
