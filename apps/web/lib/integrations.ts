...
export function integrationCatalog(env: Record<string, string | undefined> = process.env): IntegrationInfo[] {
  ...
  return [
    ...
    { key: "slack", name: "Slack", category: "Productividad", description: "Avisos de escalados y aprobaciones al equipo.", state: "coming_soon" },
    {
      ...
      description: "Disponibilidad y reserva de citas mediante OAuth.",
      state: env.GOOGLE_CLIENT_ID && env.GOOGLE_CLIENT_SECRET ? "available" : "not_configured",
      note: env.GOOGLE_CLIENT_ID && env.GOOGLE_CLIENT_SECRET ? undefined : "Requiere el cliente OAuth de la agencia (GOOGLE_CLIENT_ID, GOOGLE_CLIENT_SECRET).",
    },
